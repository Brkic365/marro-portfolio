...
const IMAGE_EXTENSION = "webp"; // Change if using "webp" or "webp"

export default async function Service({ params }: { params: { service: string } }) {
  const searchParams = useSearchParams();
  const serviceProjectIndexSearchParam = searchParams.get("q");

  const { service } = await params;

  if (!serviceProjectIndexSearchParam) return null;

  const serviceProject = services[service].examples[Number(serviceProjectIndexSearchParam)-1];

  if (!serviceProject) return null;

  // Generate image filenames dynamically
  const images = Array.from({ length: serviceProject.numImages }, (_, i) => 
    `/images/services/${service}/${serviceProject.imagesDir}/${i + 1}.${IMAGE_EXTENSION}`
  );

  // Calculate how many images each column should get
  const totalImages = images.length;
  const col2Count = Math.ceil(totalImages / 3); // Middle column gets the most
  ...
  return (
    <main className={styles.servicePage} style={{backgroundColor: serviceProject.bgColor ? serviceProject.bgColor : "#F2F2F2"}}>
      <h1 style={{
              background: `radial-gradient(circle, ${serviceProject.mainColor}, ${serviceProject.secondaryColor || "#F7F6F2"})`,
              WebkitBackgroundClip: "text",
              WebkitTextFillColor: "transparent",
              display: "inline-block" // Ensures background applies properly
            }}>{serviceProject.name}</h1>
